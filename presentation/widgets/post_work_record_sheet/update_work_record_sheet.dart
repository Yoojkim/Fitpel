...
import 'package:fitple/features/work_record/presentation/widgets/post_work_record_sheet/update_button_widget.dart';
import 'package:fitple/features/work_record/presentation/widgets/post_work_record_sheet/update_work_record_sheet_bar.dart';
...
class UpdateWorkRecordSheet extends ConsumerStatefulWidget {
  const UpdateWorkRecordSheet({super.key});

  @override
  ConsumerState<ConsumerStatefulWidget> createState() =>
      UpdateWorkRecordSheetState();
}

class UpdateWorkRecordSheetState extends ConsumerState<UpdateWorkRecordSheet> {
  @override
    ...
    return Container(
      ...
      child: SafeArea(
        child: SizedBox(
          ...
          child: Column(
            children: [
              const UpdateWorkRecordSheetBar(),
              Expanded(
    ...
    return GestureDetector(
      ...
      child: StandardSheetPartWidget.createStandardSheetPartWidgetWithStrValue(
        ...
        value: FormatterUtil.getKoreanFormattedFullDate(
          ref.read(workInfoControllerProvider).date!,
        ),
      ),
    );
  }
  ...
  Widget getSelectedEmployeeWidget() {
    ...
    return GestureDetector(
      onTap: () async {
        ...
        if (newEmployee != null && newEmployee.id != employee.id) {
          ref
              .read(workInfoControllerProvider.notifier)
  ...
  Widget getRoleWidget() {
    ...
    return GestureDetector(
      onTap: () {
        context.push(
          AppRoute.workRecordroleSelect.path,
        );
      },
      child: StandardSheetPartWidget.createStandardSheetPartWidgetWithStrValue(
        type: '역할',
  ...
  Widget getSumWidget() {
    ...
    if (ref.read(workInfoControllerProvider.notifier).canCalculateSalary()) {
      ...
      salary = WorkRecordSumUtil.getTimeAndSalaryPerWorkInfo(
          workInfo.date!, workInfo.start!, workInfo.end!, workInfo.employee!);
    }
